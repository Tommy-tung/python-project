...
class data_preparation : 
    ...
    def distress_pre(self) : 
        table = self.data.pivot(index = '公司', columns = '年', values = '常續性稅後淨利')
    ...
    def create_data(self) :
        ...
        for var in df.columns[3:] : 
            table = df.pivot(index = '公司', columns = '年',values = var).rolling(3, min_periods = 3, axis = 1).mean()
            mean = []
            period = []
            company = []
            for i in range(len(table)) : 
        ...
        self.data_full = pd.concat([pd.DataFrame([company,period], index = ['company','period']).T,pd.DataFrame(self.data_full, index = df.columns[3:].tolist()).T],axis = 1)

    
    def creat_X_Y(self) : 
        data = pd.concat([self.data_full, self.distress], axis = 1).dropna(axis = 0)
        self.X = data.iloc[:,:-4].reset_index()
        self.Y = pd.DataFrame(list(np.where((data.iloc[: ,-2] < float(0)) & (data.iloc[:, -1]< float(0)) ,1,0)), columns = ['distress'])
        
        
    def creat_train_test(self, ratio) : 
        a = random.randint(1,1000)
        self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(self.X.iloc[:,3:], self.Y, test_size = ratio, random_state = a)
